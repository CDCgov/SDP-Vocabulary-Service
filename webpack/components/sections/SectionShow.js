    ...
    return(
      <div className="static-modal">
        <Modal animation={false} show={this.state.showPublishModal} onHide={()=>this.setState({showPublishModal: false})} role="dialog" aria-label="Publish Confirmation Modal">
          ...
          <Modal.Footer>
            {this.state.publishOrRetire === 'Retire' && <Button onClick={() => this.props.retireSection(this.props.section.id)} bsStyle="primary">Confirm Retire</Button>}
            {this.state.publishOrRetire === 'Publish' && <Button onClick={() => this.props.publishSection(this.props.section.id)} bsStyle="primary">Confirm Publish</Button>}
