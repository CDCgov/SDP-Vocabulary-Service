            ...
            <div className="result-analytics">
              <ul className="list-inline">
                ...
                <li className={`result-timestamp ${(this.props.programVar || isSimpleEditable(result, this.props.currentUser)) && 'list-program-var'}`}>
                  ...
                  <p><text className="sr-only">Item Version Number: </text>version {displayVersion(result.version, (result.mostRecentPublished || result.mostRecent))} | <text className="sr-only">Item type: </text>{type.replace('_s','S').replace('section_','').replace('survey_','').replace('nested_','').replace('_dropped','').replace('nested','').replace('item','question')}</p>
                  {isSimpleEditable(result, this.props.currentUser) ? (
