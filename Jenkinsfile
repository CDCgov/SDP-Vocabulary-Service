    ...
    stage('Run Tests') {
      steps {
        ...
        */
        ...
        timeout(time: 5, unit: 'MINUTES') {
          sh 'oc process openshift//elasticsearch-ephemeral -l name=${esname} ELASTICSEARCH_SERVICE_NAME=${esname} NAMESPACE=trusted-images ELASTICSEARCH_IMAGE=elasticsearch ELASTICSEARCH_VERSION=6.8.10 | oc create -f -'
          waitUntil {
            script {
              sleep time: 15, unit: 'SECONDS'
