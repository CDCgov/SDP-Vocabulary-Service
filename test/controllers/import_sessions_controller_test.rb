...
class ImportSessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'create an import session' do
    post import_sessions_url(format: :json), params: { import_session:
      { spreadsheet: fixture_file_upload('./test/fixtures/files/TestMMG.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', :binary) } }
  ...
  test 'create an import session with a non-Excel file' do
    post import_sessions_url(format: :json), params: { import_session:
      { spreadsheet: fixture_file_upload('./test/fixtures/files/jupiter_response.json', 'application/json', :binary) } }
    assert_response :success
    response_json = JSON.parse(@response.body)
  ...
  test 'create an import session and create survey' do
    ...
    assert_difference 'Survey.count' do
      put import_session_url(is_id), params: { import_session: { request_survey_creation: true } }
    end
